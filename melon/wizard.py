...
def wizard_proxy():
    ...
    print(
        f"\t 1. Go to https://{plex_host}:{plex_port}/web/index.html#!/settings/server/{id}/settings/network"
    )
    print("\t 2. Click 'Show advanced' at the top of the page")
    print("\t 3. Scroll down until you find the 'Custom server access URLs'")
    print(f"\t 4. Enter https://{proxy_host} in the text box, and click Save Changes.")
    print(
        f"\t\t Note: https://{proxy_host} should be accessible from the internet. You may need to forward port {proxy_port if proxy_port else '443'} to this machine on your router."
    )
    print(
        f"\t 5. Go to https://{proxy_host}. If everything worked, you should see your Plex server!"
    )
    print("\n")
